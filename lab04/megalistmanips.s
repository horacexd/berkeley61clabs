...
map:							# here
        addi    sp, sp, -12		# M1: change -12 to -16
        sw      ra, 0(sp)
        sw      s1, 4(sp)		
        sw      s0, 8(sp)
...
mapLoop:								# here
        lw    	t1, 0(s0)              	# load the address of the array of current node into t1 || Mistake 3: Here we need to dereference s0 and get value, which is the address of array
        lw      t2, 4(s0)               # load the size of the node's array into t2

        addi	t3, x0, 4
        mul		t3, t3, t0				# Mistake 4: calculate the offset of new element of array address
        add     t3, t3, t1            	# offset the array address by the count
        ...
        sw		t1, 4(sp)
        sw		t2, 8(sp)
        sw		t3, 12(sp)				# move sp to save callee register
		...
		lw		t1, 4(sp)
        lw		t2, 8(sp)
        lw		t3, 12(sp)				# restore the callee value after function call
        addi 	sp, sp, 16
        ...
        lw      a0, 8(s0)				# load the address of the next node into a0 ## Mistake 1
        #
        mv      a1, s1               	# put the address of the function back into a1 to prepare for the recursion # Mistake 2: s1 is function address, we need to be aware of that
        								# there is no value in function address // mv is equal to add a1, s1, x0
